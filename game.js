...
function checkBoss(boss) {
  ...
  if (boss.hp <= 0) {
    ...
    if (boss.hp < boss.hpMax*0.4 && boss.fase===1) { boss.fase = 2; boss.bodyM.color.set(0xffaaaa); boss.pl.color.set(0xff4400); boss.pl.intensity = 3; showCombo('FÚRIA DO CHEFE!'); }
  }
}
...
function updateBoss(dt, boss, player, scene) {
  ...
  if (player.pos.distanceTo(boss.mesh.position) < 2.0) { if (!player.onGround && player.vel.y < 0) { boss.hp -= 1; boss.flashTimer = 0.1; checkBoss(boss); player.vel.y = 8; } else { player.takeDamage(); } }
